...
async function runPipelineSearch(query, isLucky = false) {
    ...
    try {
        ...
        console.error("Backend API query failed:", err);
        currentResults = [];
        updateDynamicTabCounts();
        if (!isLucky && resultsMain) {
            resultsMain.innerHTML = `
                <div class="results-info">Search backend is unavailable right now.</div>
                <div style="margin-top:20px; color:var(--secondary-text)">
                    This usually means the server is waking up from being idle (free hosting spins
                    down after inactivity) or is mid-deploy. Try again in a few seconds.
                </div>`;
        }
        if (pipelineStatus) pipelineStatus.innerText = "Status: Backend unreachable.";
    }
}
