...
const { fetchDictionaryDefinition } = require('./src/search/dictionary');
...
app.get('/api/search', async (req, res) => {
  ...
  try {
    ...
    const [result, dictionaryCard] = await Promise.all([
      search(query, Number(req.query.limit) || 20),
      isSingleWord ? fetchDictionaryDefinition(words[0]) : Promise.resolve(null)
    ]);

    if (dictionaryCard) result.results.unshift(dictionaryCard);
    res.json(result);
  } catch (err) {
